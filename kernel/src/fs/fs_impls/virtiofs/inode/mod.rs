...
pub(super) struct VirtioFsInode {
    ...
    ///
    /// It is intentionally kept outside `inner` because `VirtioFsInode` is also
    /// the page-cache backend for its own page cache. Page-cache operations
    /// may call back into this inode through the backend interface, so reading
    /// EOF must not require acquiring `inner` again.
    size: AtomicUsize,
    ...
    fn create(&self, name: &str, type_: InodeType, mode: InodeMode) -> Result<Arc<dyn Inode>> {
        ...
        let entry_out = match type_ {
            InodeType::File => fs.session().do_fuse_op(
                parent_nodeid,
                MknodOperation::new(
                    MknodIn::new(InodeType::File as u32 | u32::from(mode.bits()), 0),
                    name,
                ),
            )?,
...
impl FileOps for VirtioFsInode {
    ...
    fn write_at(
        ...
        let handle = self.open_transient_handle(AccessMode::O_RDWR)?;
