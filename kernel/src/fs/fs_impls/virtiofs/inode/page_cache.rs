...
use aster_fuse::{FuseCompletion, ReadIn, WriteFlags, WriteIn};
...
use crate::{
    fs::file::AccessMode,
    prelude::*,
    vm::page_cache::{CachePageExt, LockedCachePage, PageCacheBackend, clear_writing_back},
};
    ...
    fn write_page_async(
        ...
        let handle = match self.writable_page_handle() {
            ...
            Err(err) => {
                ...
                clear_writing_back(&locked_page);
                return Err(err);
            }
        };
        ...
        let complete_fn = move |status| {
            clear_writing_back(&complete_page);
